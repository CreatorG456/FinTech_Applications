...
################################################################################
# * Generate a new Ethereum account instance by using my mnemonic seed phrase

# * Fetch and display the account balance associated with my Ethereum account
# address.
...
# customer's (in this case, my) HD wallet and Ethereum account.
...
#  pass it my Ethereum `account.address`.

# @TODO
# Call `get_balance` function and pass it my account address
...
# that will allow a customer (my, in this case) to send an Ethereum blockchain
# transaction that pays the hired candidate. To accomplish this, locate the
# code that reads `if st.sidebar.button("Send Transaction")`. I will need to
# add logic to this `if` statement that sends the appropriate information to
# the `send_transaction` function (which my imported from the `crypto_wallet`
# script file). Inside the `if` statement, add the following functionality:

    # * Call the `send_transaction()` function and pass it three parameters:
        # - my Ethereum `account` information. (Remember that this `account`
...
if st.sidebar.button("Send Transaction"):
    ...
    # my `account`, the `candidate_address`, and the `wage` as parameters
    ...
    # Celebrate my successful payment
    st.balloons()
...
# 1. From my terminal, navigate to the project folder that contains
# my `.env` file and the `fintech_finder.py` and `crypto_wallet.py` files.
# Be sure to activate my Conda `dev` environment if it is not already active.

# 2. To launch the Streamlit application,
# type `streamlit run fintech_finder.py`.

# 3. On the resulting webpage, select a candidate that my would like to hire
# from the appropriate drop-down menu. Then, enter the number of hours that my
# would like to hire them for. (Remember, my do not have a lot of ether in
# my account, so my cannot hire them for long!)

# 4 Click the Send Transaction button to sign and send the transaction with
# my Ethereum account information. If the transaction is successfully
# communicated to Ganache, validated, and added to a block,
# a resulting transaction hash code will be written to the Streamlit
# application sidebar.

# 5. Navigate to the Ganache accounts tab and locate my account (index 0).
    # * Take a screenshot of the address, balance, and transaction (TX) count.
    # Save this screenshot to the README.md file of my GitHub repository for
